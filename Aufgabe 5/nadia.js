...
function erstelleBienen(_menge) {
    for (var i = 0; i < _menge; i++) {
        var x = Math.random(0, crc2.canvas.width);
        var y = Math.random(0, crc2.canvas.height);
        biene(x, y);
    }
}
function plusBiene(_event) {
    for (var i = 0; i < 1; i++) {
        var x = Math.random(_event.offsetX, _event.offsetX);
        var y = Math.random(_event.offsetY, _event.offsetY);
        erstelleBienen(x, y);
    }
    function drawSweetRandom() {
        var numberSweets = Math.floor((Math.random() * 30) + 5);
        var _x;
        var _y;
        var _p;
        var _radius;
